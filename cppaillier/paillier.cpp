...
int_t PaillierPrivateKey::randomE() const{
    int_t ret;
    do {
        ret = getRandomNumber(mpz_sizeinbase((this->n).get_mpz_view(), 2)/3);
    } while (ret == 0 || ret >= pow(n, 1.0 / 3.0) );
    return ret;
}
...
PaillierEvaluationKey PaillierPrivateKey::generateEvaluationKey() const{
    const int_t phi = (this->p - 1)*(this->q - 1);
    const int_t n = this->n;
    const int_t e1 = randomE();
    const int_t e2 = randomE();
    const int_t z = 1;
    const int_t ck0 = e1*powm(phi, z, n);
    const int_t ck1 = e2*invertm(pow(phi,z),n);
    return PaillierEvaluationKey(ck0, ck1, n);
}
...
PaillierEvaluationKey::PaillierEvaluationKey(const int_t &ck0, const int_t &ck1, const int_t& n){
    this->n = n;
    this->_n2 = pow(this->n, 2);
    this->g = this->n+1;
    this->pk = PaillierPublicKey(this->n);
...
int_t PaillierEvaluationKey::compare(const int_t &enc_n, const int_t &enc_m, const PaillierEvaluationKey &ek){
    int_t sub_res= enc_n*invertm(enc_m, ek._n2) % ek._n2;
    int_t l = (powm(sub_res, ek.ck0, ek._n2) - 1) / ek.n;
    return (l*ek.ck1 % ek.n+ ek.n/int_t{2}) % ek.n - ek.n/int_t{2};
};
int_t PaillierEvaluationKey::compare(const int_t &enc_n, const int_t &enc_m) const{
    return PaillierEvaluationKey::compare(enc_n, enc_m, *this);
};

int_t PaillierPrivateKey::decrypt(const int_t &ciphertext) const{
    int_t n = this->n;
    int_t phi = (this->p - 1)*(this->q - 1);
    int_t dec_Paillier = (PaillierPrivateKey::L_function(powm(ciphertext, phi, this->_n2), n)*invertm(phi, n)) % n;
    int_t res = ((dec_Paillier + n/int_t{2}) %n) - n/int_t{2};
    return res;
}
/*int_t PaillierPrivateKey::decrypt(const int_t &ciphertext) const{
